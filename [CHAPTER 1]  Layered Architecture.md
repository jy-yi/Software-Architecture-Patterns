...
# 계층화된 아키텍처
 ...
 계층화된 아키텍처 패턴의 강력한 특징 중 하나는 요소들 간의 책임에 대한 분리다. 특정 계층 내의 컴포넌트는 해당 계층과 관련된 논리만 다룬다. 예를 들어, 표현 계층의 컴포넌트는 표현 로직만을 다루지만, 비즈니스 계층에 포함되는 컴포넌트는 비즈니스 로직만을 다룬다. 이러한 유형의 컴포넌트 분류는 여러분의 아키텍처에 효과적인 역할과 책임(R&R) 모델을 쉽게 구축할 수 있게 하며, 또한 잘 정의된 컴포넌트는 인터페이스와 제한된 컴포넌트 인터페이스의 범위 때문에 아키텍처 패턴을 사용하면 애플리케이션을 더 쉽게 개발, 테스트, 유지보수 할 수 있다.
